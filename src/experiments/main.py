...
def main():
    ...
    # 初始化验证器
    verifier = ModelVerifier(device=device)  # Add this line
    ...
    print(f"- 学习率: {args.lr}")  # Changed from args.learning_rate to args.lr
    ...
    for i in range(args.num_baseline_models):
        ...
        if os.path.exists(model_path) and args.load_models and not args.force_retrain:
            ...
            model_stats.update({
                ...
                "epochs": args.epochs,
                "batch_size": args.batch_size,
                "lr": args.lr,  # 修改这里，从"learning_rate"改为"lr"
                "is_distilled": False,
    ...
    if args.train_distilled:
        ...
        # 使用一个独立训练的模型作为目标模型
        # 训练目标模型时使用与基线模型完全相同的参数
        target_model_id = f"target_{int(time.time())}"
        target_model_path = os.path.join(dirs["models_dir"], f"{target_model_id}.pth")
        
        # 创建新的训练器实例，确保使用新的随机种子
        trainer = ModelTrainer(
            device=device,
            batch_size=args.batch_size,
            epochs=args.epochs,
    ...
    verification_result = verifier.verify_black_box(
        target_model=target_model,  # Add target model
        baseline_models=baseline_models,  # Add baseline models
        test_loader=test_loader,  # Keep test data loader
        confidence_level=args.confidence  # Add confidence level
    )
    ...
    # Ensure kl_matrix is correctly initialized and populated
    ...
    for i in range(len(baseline_models)):
        for j in range(i + 1, len(baseline_models)):
            # 计算对称KL散度: KL_sym(M_i, M_j) = 1/2[KL(P_i||P_j) + KL(P_j||P_i)]
            kl_ij = kl_matrix[i][j]
            kl_ji = kl_matrix[j][i]
    ...
    # 计算置信区间 - 使用t分布的临界值
    # 对于95%置信水平和自由度为9(10个KL值-1)，t临界值约为2.262
    ...
    # 进行t检验 - 独立样本t检验
    # 计算t统计量
    # t = (μ_target - μ_base) / sqrt((σ²_base/n_base) + (σ²_target/n_target))
    n_base = len(baseline_kl_values)  # 基线样本量 = 10 (5个模型的两两组合)
    n_target = len(target_kl_values)  # 目标样本量 = 5 (目标模型与5个基线模型的KL值)
    ...
    verification_result.update({
        ...
        "zkp_metrics": {
            "proof_generation_time": sum([p.get("generation_time", 0) for p in zkp_proofs]) if zkp_proofs else 0,
            "proof_size": sum([p.get("proof_size", 0) for p in zkp_proofs]) if zkp_proofs else 0,
            "verification_time": sum([p.get("verification_time", 0) for p in zkp_proofs]) if zkp_proofs else 0,
            "num_proofs": len(zkp_proofs) if zkp_proofs else 0
        }
    })
    
    # 创建报告生成器并生成验证报告
    # 在调用generate_all_reports之前，确保准备好baseline_stats和target_stats
    # 从模型输出中提取分布信息
    baseline_stats = []
    for model in baseline_models:
        with torch.no_grad():
            ...
            baseline_stats.append(np.concatenate(outputs))
    ...
    with torch.no_grad():
        ...
        for data, _ in test_loader:
            ...
            outputs.append(torch.softmax(output, dim=1).cpu().numpy())
        target_stats.append(np.concatenate(outputs))
    ...
    # 从KL散度计算中提取分布信息
    # 创建baseline_stats和target_stats用于报告生成
...
# 在验证完成后添加
# 假设 verify_black_box 是一个返回验证结果的函数
verification_result = verifier.verify_black_box(target_model, baseline_models, test_loader, confidence_level=0.95)

print(f"KL值: {verification_result['target_kl_mean']:.4f}")
print(f"证明生成时间: {proof_generation_time:.2f}秒")
print(f"证明验证时间: {proof_verification_time:.2f}秒")
print(f"证明大小: {proof_size_bytes/1024:.2f}KB")
print(f"可行性: {'高' if verification_result['passed_verification'] else '低'}")
print(f"置信区间: [{verification_result['confidence_interval'][0]:.4f}, {verification_result['confidence_interval'][1]:.4f}]")
