...
class ModelTrainer:
    ...
    def train_with_zkp_sampling(self, train_loader, test_loader, zkp_generator, sample_rate=None):
        """
        训练模型并进行ZKP抽样验证，支持多种抽样次数
        ...
        """
        # 默认抽样次数
        sample_rates = [sample_rate] if sample_rate is not None else [10, 20, 30, 40, 50]
        ...
        for sample_rate in sample_rates:
            # 设置抽样次数
            zkp_generator.set_sample_count(sample_rate)
            ...
            # 获取证明文件大小 - add check for proof_path existence
            proof_size = os.path.getsize(proof.get("proof_path", "")) if proof and os.path.exists(proof.get("proof_path", "")) else 0
            
            # 测量验证时间 - add check for proof_path existence
            verification_time = 0
            if proof and "proof_path" in proof and "public_path" in proof:
                start_time = time.time()
                verification_result = zkp_generator.verify_proof(proof["proof_path"], proof["public_path"])
                verification_time = time.time() - start_time
            
            # 存储性能指标
            zkp_metrics["proof_generation_times"].append(proof_generation_time)
            zkp_metrics["proof_sizes"].append(proof_size)
            zkp_metrics["verification_times"].append(verification_time)
        
        # 在控制台中以表格形式展示性能指标
        print("\n性能指标表格:")
        print("抽样次数\t证明生成时间(秒)\t证明规模(字节)\t验证时间(秒)")
        print("-" * 70)
        for i, sample_rate in enumerate(sample_rates):
            print(f"{sample_rate}\t{zkp_metrics['proof_generation_times'][i]:.4f}\t\t{zkp_metrics['proof_sizes'][i]}\t\t{zkp_metrics['verification_times'][i]:.4f}")
        ...
        return model, stats, zkp_metrics
